        ...
        if let Ok(out_dir) = std::env::var("OUT_DIR") {
            ...
            if let Some(debug_dir) = out_path.ancestors().nth(3) {
                ...
                if let Some(target_dir) = debug_dir.parent() {
                    let _ = copy_dll_recursive(target_dir, &deps_dir);
                }
            }
        }
    }
...
fn copy_dll_recursive(src: &std::path::Path, dst_deps: &std::path::Path) -> std::io::Result<()> {
    if src.is_dir() {
        for entry in std::fs::read_dir(src)? {
            let entry = entry?;
            let path = entry.path();
            if path.is_dir() {
                let _ = copy_dll_recursive(&path, dst_deps);
