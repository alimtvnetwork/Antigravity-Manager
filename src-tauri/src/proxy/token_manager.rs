...
impl TokenManager {
    ...
    pub async fn reload_account(&self, account_id: &str) -> Result<(), String> {
        ...
        match self.load_single_account(&path).await {
            Ok(Some(token)) => {
                // If account has quota restored (>0%), clear rate limit tracker
                    ...
                    if let Some(wait_sec) = min_wait {
                        if wait_sec <= 2 {
                            ...
                            if let Some(t) = retry_token {
                                ...
                                // Retry selecting account (must re-verify rate limit status, strictly forbidding accounts with exhausted weekly quota or long locks)
    ...
    async fn mark_rate_limited_async_with_mode(
        ...
        // Key gatekeeper 1: Only genuine upstream 429 (quota exhausted/rate limit) and 529 (Overloaded) enter cooldown; 500/503/404 must never enter cooldown!
        if status != 429 && status != 529 {
            return;
        }

        // Key gatekeeper 2: Internal error text (All accounts limited / No accounts available / Token pool is empty, etc.) must never trigger recursive self-lockout!
    ...
    fn sync_zero_quota_circuit_breaker(&self, account_id: &str, account: &serde_json::Value) {
        ...
        // 2. Fall back to models quota check
        if !lock_on_zero {
            return;
        }
        if let Some(models) = quota.get("models").and_then(|m| m.as_array()) {
...
mod tests {
    ...
    fn test_sync_zero_quota_circuit_breaker_later_deadline_and_recovery() {
        ...
        // 1. Both weekly quota and 5h quota are 0 (exhausted)
        // Weekly quota reset_time is 5 days later, 5h reset_time is 2 hours later
        ...
        // Even if lock_on_zero is false, exhausted weekly quota unconditionally locks until weekly reset time
        ...
        // 2. Simulate provider early reset: weekly quota restored to 100%, 5h quota still 0
        // If lock_on_zero is enabled, automatically align to the shorter 5h reset time (2 hours)
        {
        ...
        // 3. Simulate 5h quota also fully restored (all quotas positive)
