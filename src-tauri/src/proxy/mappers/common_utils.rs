...
pub fn resolve_request_config(
    ...
    // [FIX] Never downgrade models silently to gemini-2.5-flash
...
pub fn inject_google_search_tool(body: &mut Value, _mapped_model: Option<&str>) {
    if let Some(obj) = body.as_object_mut() {
        ...
        if let Some(tools_arr) = tools_entry.as_array_mut() {
            ...
            // [STABILITY GUARD] If client already defines function tools (functionDeclarations / function_declarations),
            // do not inject googleSearch tool to prevent unhandled client tool dispatch errors.
            if has_functions {
                tracing::debug!(
