...
/// Clean system prompt noise from user messages (<system-reminder>, <environment>, [System...])
/// Extract genuine user input ensuring deterministic session fingerprints across turns
pub fn sanitize_user_text_for_fingerprint(raw: &str) -> String {
    let mut s = raw.to_string();

    // 1. Strip <system-reminder>...</system-reminder> tags and inner content
    ...
    // 2. Strip <environment>...</environment> tags and inner content
...
impl SessionManager {
    ...
    pub fn extract_session_id(request: &ClaudeRequest) -> String {
        ...
        if !content_found {
            // Fallback 1: Extract first user message text
            for msg in &request.messages {
                if msg.role == "user" {
        ...
        if !content_found {
            // Fallback 2: Hash last message if no user message found
            if let Some(last_msg) = request.messages.last() {
                hasher.update(format!("{:?}", last_msg.content).as_bytes());
            }
        }

        // [NEW] Fuse multi-dimensional environment features (System Prompt digest + Tools digest)
        // Resolves fingerprint collisions when multiple windows share identical first messages
        hasher.update([0xff]);
    ...
    pub fn extract_openai_session_id(request: &OpenAIRequest) -> String {
        ...
        // [NEW] Fuse environment features (System/Developer message digest + Tools digest)
        hasher.update([0xff]);
        for msg in &request.messages {
    ...
    pub fn extract_gemini_session_id(request: &Value, _model_name: &str) -> String {
        ...
        // [NEW] Fuse environment features (systemInstruction digest + tools digest)
        hasher.update([0xff]);
