...
const SAMPLE_INBOUND_COMMANDS = [
    {
        title: 'Antigravity AI Prompt Injection (Subject-Driven)',
        description: 'Command in Subject line, ONLY the pure prompt inside the email Body.',
        subject: '* | prompt | proj-Antigravity-Manager',
        body: `Please analyze recent build failures in .github/workflows/ci.yml and optimize test run times.`,
        command: `Please analyze recent build failures in .github/workflows/ci.yml and optimize test run times.`,
    },
    {
        title: 'PowerShell Service Inspection (Direct)',
        description: 'Inspect running system, WSL, and Docker services directly in PowerShell (no prefix).',
        subject: '* | ps | Get-Service',
        body: `Get-Service -Name '*wsl*', '*docker*', '*hyper*' -ErrorAction SilentlyContinue | Select-Object Name, Status, StartType`,
        command: `Get-Service -Name '*wsl*', '*docker*', '*hyper*' -ErrorAction SilentlyContinue | Select-Object Name, Status, StartType`,
    },
    {
        title: 'PowerShell Top Memory Processes (Direct)',
        description: 'Identify the top memory-consuming processes directly in PowerShell (no prefix).',
        subject: '* | ps | Get-Process',
        body: `Get-Process | Sort-Object -Property WorkingSet64 -Descending | Select-Object -First 10 -Property Name, Id, @{Name="MB";Expression={[math]::round($_.WorkingSet64/1MB,2)}}`,
        command: `Get-Process | Sort-Object -Property WorkingSet64 -Descending | Select-Object -First 10 -Property Name, Id, @{Name="MB";Expression={[math]::round($_.WorkingSet64/1MB,2)}}`,
    },
    {
        title: 'GitMap Repository Health Scan',
        description: 'Trigger autonomous repository health and branch scanning via GitMap CLI.',
        subject: '* | gitmap | status',
        body: `gitmap status`,
        command: `gitmap status`,
    },
    {
        title: 'Interactive Help & Command Reference',
        description: 'Request a 100% plaintext interactive command cheat sheet by email.',
        subject: '* | help',
        body: `help`,
        command: `help`,
    },
    {
        title: 'Instance / Account Fast-Forward Switch',
        description: 'Rotate active Antigravity workspace to the next healthiest account and push running prompt.',
        subject: '* | switch',
        body: `switch`,
        command: `switch`,
    },
