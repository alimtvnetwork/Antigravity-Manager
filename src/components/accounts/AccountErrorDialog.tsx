...
export default function AccountErrorDialog({ account, onClose }: AccountErrorDialogProps) {
    ...
    const extractErrorMessage = (raw: string) => {
        ...
        try {
            ...
            // Extract message following priority
    ...
    const extractActionInfo = (raw: string): { url: string | null, label: string | null } => {
        ...
        try {
            const parsed = JSON.parse(trimmed);
            // Google API links usually reside in metadata
    ...
    const handleCopyUrl = (url: string) => {
        ...
        showToast(t('accounts.validation_url_copied', 'Verification URL copied to clipboard'), 'success');
    };
        ...
        return parts.map((part, i) => {
            if (part.match(urlRegex)) {
                return (
                    ...
                    >
                        {t('accounts.click_to_verify', 'Verify')}
                ...
                <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        ...
                        {isViolation && (
                            <span className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 text-xs font-bold ring-1 ring-red-200/50 dark:ring-red-900/20">
                                <Lock className="w-3 h-3" />
                                {t('accounts.status.violation_blocked', 'Disabled due to policy violation')}
                            </span>
                        )}
                        {isDisabled && (
                        ...
                        {(isValidationBlocked || isVerificationNeeded) && (
                            <span className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 text-xs font-bold ring-1 ring-amber-200/50 dark:ring-amber-900/20">
                                ...
                                {t('accounts.status.validation_required', 'Account verification required')}
                            </span>
                        )}
                    </div>
                </div>
                ...
                <div>
                    <div className="flex items-center justify-between mb-1.5 ml-1">
                        ...
                        >
                            ...
                            {showRaw ? t('common.show_parsed', 'Show Parsed') : t('common.show_raw', 'Show Raw')}
                        </button>
                    </div>
                    ...
                    {actionUrl && !showRaw && (
                        <div className="mt-3 flex flex-col sm:flex-row gap-2">
                            ...
                            >
                                <ExternalLink className="w-3 h-3" />
                                {actionLabel || (isViolation ? t('accounts.go_to_appeal', 'Appeal') : t('accounts.click_to_verify', 'Verify'))}
                            </a>
                            <button
                            ...
                            >
                                ...
                                {isViolation ? t('accounts.copy_appeal_url', 'Copy Appeal URL') : t('accounts.copy_validation_url', 'Copy Verification URL')}
                            </button>
                        </div>
                    )}
                        ...
                        <div className="mt-4 border border-blue-100 dark:border-blue-900/40 rounded-xl overflow-hidden shadow-sm">
                            ...
                            >
                                <div className="flex items-center gap-2 text-blue-700 dark:text-blue-400 font-bold text-xs">
                                    ...
                                    <span>{t('accounts.fix_guide.title', 'Terminal Self-Help Guide (Fixes certain 403 blocks)')}</span>
                                </div>
                                ...
                                <div className="p-4 text-xs space-y-4 bg-white dark:bg-base-200 text-gray-700 dark:text-gray-300 max-h-[35vh] overflow-y-auto scrollbar-thin scrollbar-thumb-blue-200 dark:scrollbar-thumb-blue-800">
                                    <div>
                                        <p className="mb-2 text-[11px] leading-relaxed">
                                            {t('accounts.fix_guide.step1_desc', 'Open Terminal and execute the following command to re-authenticate with Google ADC:')}
                                        </p>
                                        <div className="bg-gray-900 dark:bg-[#1e1e1e] text-green-400 p-2.5 rounded-lg font-mono text-[11px] flex justify-between items-center ring-1 ring-inset ring-gray-800">
                                            ...
                                            <button
                                                onClick={() => handleCopyText('gcloud auth login --update-adc', t('common.copied', 'Command copied successfully'))}
                                        ...
                                        <ul className="mt-2 text-[11px] text-gray-500 dark:text-gray-400 list-disc pl-4 marker:text-gray-300 dark:marker:text-gray-600">
                                            <li><Trans i18nKey="accounts.fix_guide.step1_li1" components={{ 1: <code /> }}>Press enter to execute, and enter <code /> when prompted to proceed.</Trans></li>
                                            <li>{t('accounts.fix_guide.step1_li2')}</li>
                                            <li><Trans i18nKey="accounts.fix_guide.step1_li3" components={{ 1: <code /> }}>Once you see <code />, authentication is complete!</Trans></li>
                                        </ul>
                                    </div>
                                    ...
                                    <div className="border-t border-gray-100 dark:border-base-300/50 pt-3">
                                        <h4 className="font-bold text-gray-800 dark:text-gray-200 mb-1.5 flex items-center gap-1.5">
                                            {t('accounts.fix_guide.step2_title', 'Clean cache and retry')}
                                        </h4>
                                        <ol className="list-decimal pl-4 space-y-2 text-[11px] text-gray-600 dark:text-gray-400 marker:text-gray-400 font-medium">
                                            <li>
                                                {t('accounts.fix_guide.step2_li1_prefix', 'Revoke existing authentication:')}
                                            ...
                                            <li>{t('accounts.fix_guide.step2_li2_prefix', 'Re-authenticate with ADC:')}<code className="bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-1 rounded ml-1">gcloud auth login --update-adc</code></li>
                                        </ol>
                                    </div>

                                    <div className="border-t border-gray-100 dark:border-base-300/50 pt-3">
                                        <h4 className="font-bold text-gray-800 dark:text-gray-200 mb-1.5 flex items-center gap-1.5">
                                            {t('accounts.fix_guide.tips_title', 'Helpful Tips')}
                                        </h4>
                                        <ul className="list-disc pl-4 space-y-1.5 text-[11px] text-gray-500 dark:text-gray-400 font-medium marker:text-gray-300">
                                            <li><Trans i18nKey="accounts.fix_guide.tip1" components={{ 1: <code /> }}>If 403 persists, execute <code /> in terminal to reset environment variables.</Trans></li>
                                            <li><Trans i18nKey="accounts.fix_guide.tip2" components={{ 1: <strong /> }}>In production environments, using a service account JSON key with <strong /> is recommended for stability.</Trans></li>
                                            <li><Trans i18nKey="accounts.fix_guide.tip3" components={{ 1: <a href="https://console.cloud.google.com/" target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:text-blue-600 hover:underline" /> }}>If operations fail, check the Generative Language API status in <a /> to ensure permissions are not frozen.</Trans></li>
                                            <li><Trans i18nKey="accounts.fix_guide.tip4" components={{ 1: <code /> }}>You can also run <code />; as long as no error appears, re-authorizing the account should resolve it.</Trans></li>
                                        </ul>
                                    </div>
                                </div>
                            )}
