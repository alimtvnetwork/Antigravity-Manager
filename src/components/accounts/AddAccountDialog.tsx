...
function AddAccountDialog({ onAdd, showText = true }: AddAccountDialogProps) {
    ...
    const handleAction = async (
        ...
        try {
            ...
            // Delayed close so user sees success state
            setTimeout(() => {
                setIsOpen(false);
                resetState();
            }, 1500);
            ...
            // If refresh_token is missing, display full error details
            if (errorMsg.includes('Refresh Token') || errorMsg.includes('refresh_token')) {
                setMessage(errorMsg);
            } else if (errorMsg.includes('Tauri') || errorMsg.toLowerCase().includes('environment')) {
                // Environment error
    ...
    const handleSubmit = async () => {
        ...
        // 2. If JSON parsing yielded no result, try regex extraction
        if (tokens.length === 0) {
        ...
        if (successCount === tokens.length) {
            ...
            // Keep open so user can review results
        } else {
            // All failed
            setStatus('error');
    ...
    const handleOAuthWeb = async () => {
        try {
            ...
            // 1. Fetch URL (points to /auth/callback)
            ...
            setOauthUrl(url); // Ensure link is visible in UI for manual copy

            // 2. Open new tab (better user experience in Web mode)
            ...
            const handleMessage = async (event: MessageEvent) => {
                ...
                if (event.data?.type === 'oauth-success') {
                    ...
                    // 4. Refresh account list upon success
                    await fetchAccounts();
            ...
            const timer = setInterval(() => {
                if (popup.closed) {
                    clearInterval(timer);
                    window.removeEventListener('message', handleMessage);
                    if (statusRef.current === 'loading') { // User closed window while loading
                        setStatus('idle');
                        setMessage('');
                    }
                }
    ...
    const handleManualSubmit = async () => {
        ...
        try {
            ...
            // Align refresh logic for Web mode
            if (!isTauri()) {
                setTimeout(async () => {
                    await fetchAccounts();
                    setIsOpen(false);
                ...
                >
                    ...
                    <div className="bg-white dark:bg-base-100 text-gray-900 dark:text-base-content rounded-2xl shadow-2xl w-full max-w-lg p-6 relative z-[10] m-4 max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="bg-gray-100 dark:bg-base-200 p-1 rounded-xl mb-6 grid grid-cols-3 gap-1">
                            <button
                                className={`py-2 px-1.5 sm:px-3 rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 ${activeTab === 'oauth'
                            ...
                            <button
                                className={`py-2 px-1.5 sm:px-3 rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 ${activeTab === 'token'
                                    ? 'bg-white dark:bg-base-100 shadow-sm text-blue-600 dark:text-blue-400'
                                    : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200 hover:bg-gray-200/50 dark:hover:bg-base-300'
                                    } `}
                            ...
                            <button
                                className={`py-2 px-1.5 sm:px-3 rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 ${activeTab === 'import'
                                    ? 'bg-white dark:bg-base-100 shadow-sm text-blue-600 dark:text-blue-400'
                                    : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200 hover:bg-gray-200/50 dark:hover:bg-base-300'
                                    } `}
