...
*   **版本历史记录 (Version History)**:
    ...
    *   **v4.72.0 (2026-09-24)**:
        -   **[Release v4.72.0: 发布页面双栏一键安装布局、Release 工作流子进程变量注入、PowerShell 交互历史提取版本与 Win32/WebView2 DWM 任务栏缩略图防白屏重绘] 发布页独立最新与版本化代码块、安装器版本队列严格锁定防漂移、任务栏实时缩略图与窗口重绘链路全线贯通**:
            ...
            -   **GitHub Actions 工作流子进程环境变量丢失修复 (`02-spec/22-app-issues/10-blank-ui-dwm-occlusion-and-versioned-installer-rca.md`)**: 修复 `.github/workflows/release.yml` 在 `Collect release files and generate checksums` 独立 step 中未传递 `VERSION` 导致 `VER` 为空，进而使 `sed "s/__PINNED_VERSION__/$VER/g"` 将占位符替换为空字符串的重大缺陷。显式注入环境变量，确保构建出的 `install.ps1` 与 `install.sh` 100% 烙印当前发布的精确版本号。
            -   **PowerShell 交互历史版本自动提取与容错**: 在 `install.ps1` 的 `Get-InvocationHistoryCandidates` 中深度扩展支持 `Get-History`、`[Microsoft.PowerShell.PSConsoleReadLine]::GetHistoryItems()` 及 PSReadLine 历史持久化文件，彻底解决通过 `irm .../vX.Y.Z/install.ps1 | iex` 管道执行时 `$MyInvocation.Line` 为空导致版本丢失的难题，精准提取下载 URL 中的目标版本。
            -   **固定版本候选队列严格隔离与版本漂移根除**: 在 `install.ps1` 与 `install.sh` 中重构候选队列逻辑，当检测到指定版本 (`$isPinned = $true`) 时，立即禁用 CDN 清单过期检测与 GitHub API 全量拉取逻辑，绝不在后台擅自向队列塞入未请求的最新版本（如 `v4.71.1`）；若 API 受限，自动拼装确定的 GitHub Release 资源直链，杜绝静默升级或安装损坏版本。
            -   **Win32 DWM 帧重算与 WebView2 视口重绘链路全通**: 彻底根除 Windows 10/11 任务栏悬停时显示灰色空白预览框或恢复后白屏的顽疾。在 `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS` 中增加 `--disable-features=CalculateNativeWinOcclusion,CalculateNativeWindowOcclusion` 及后台限频禁用参数；在 `force_restore_and_focus_win32` 中引入 Win32 `SetWindowPos(..., SWP_FRAMECHANGED)` 与 `RedrawWindow(RDW_INVALIDATE | RDW_INTERNALPAINT | RDW_UPDATENOW | RDW_ALLCHILDREN)` 强制重算 DWM 重定向位图；在恢复阶段执行 `window.dispatchEvent(new Event('resize'))` 并与前端 `App.tsx` 联动，确保恢复瞬间界面立即刷新。
            -   **根因分析与设计规范文档固化**: 新增规范文档 `02-spec/21-app/45-two-bar-installer-and-dwm-blank-ui-fix.md` 与根因分析报告 `02-spec/22-app-issues/10-blank-ui-dwm-occlusion-and-versioned-installer-rca.md`。
    ...
    *   **v4.71.0 (2026-09-24)**:
        -   **[Release v4.71.0: 远程邮件现代化 HTML 卡片回执、主机节点别名与本地 IP 标题标识、多空格管道容错与 IDE 智能切号凭据深层注入] RFC 2046 双重 MIME 响应卡片、主机节点标识前缀、全场景弹性管道解析、智能轮换器深度绑定与 IDE 凭据无感刷新重启**:
            ...
            -   **邮件标题全面嵌入节点别名与本地 IP 标识**: 所有由 AGM 节点分发出站的邮件主题（包括即时确认 ACK、执行结果卡片及配额告警）强制前置统一节点标识 `[<VM_ALIAS> | <LOCAL_IP>]`（例如 `[VM3 | 192.168.1.12]`）。重构 `format_reply_subject` 邮件主题格式化算法，智能剥离多重嵌套方括号与历史前缀，完美保持 Gmail / Outlook 邮件会话树（Thread）连续性。
            -   **全场景多空格与管道符指令容错解析**: 优化入站指令管道解析器，基于过滤去除首尾空格的 token 流，支持 0 空格 (`VM3|1|help`)、标准单空格 (`VM3 | 1 | help`) 及多空格制表符 (`VM3   |   1   |   help`) 的完全无歧义解析，精准解耦目标节点、实例编号与指令参数。
            -   **智能切号器 (Smart Rotator) 深度集成与 IDE 凭据全链路注入**: 彻底解决通过邮件远程切号后 IDE 无法生效的痛点。将 `agm switch`、`agm ff` 及 `agm rotate` 统一接入智能轮换引擎 (`trigger_manual_rotation_for_instance`)。当指定目标邮箱时，通过 `switch_account_to_instance` 触发完整凭据注入链路：刷新 Google OAuth 令牌、加密写入 IDE 沙箱数据库 (`state.vscdb`)、同步更新系统凭据保管库 (Keyring)、更新多实例注册表绑定，并自动重启活跃沙箱进程以无感加载新账号凭据。
            -   **根因分析与代码规范体系固化**: 输出完整根因分析报告 `.ai-memory/issues/46-email-html-replies-node-identity-smart-rotator-rca.md`，补齐单核构建内存保护策略与管道单元测试。
    *   **v4.70.0 (2026-09-24)**:
...
*   **Version History**:
    ...
    *   **v4.65.0 (2026-09-23)**:
        -   **[Release v4.65.0] Update installed app branding to Antigravity Manager Tools and bump to v4.65.0**:
            ...
            -   **Enhancements**: Update installed app branding to Antigravity Manager Tools and bump to v4.65.0.
    *   **v4.63.0 (2026-09-23)**:
