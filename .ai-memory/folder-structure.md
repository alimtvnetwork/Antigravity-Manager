...
This document defines the authoritative canonical structure of the repository, including the `.ai-memory/` AI metadata layer and the `spec/` specification tree. All AI agents MUST strictly adhere to this architecture when reading, indexing, or writing files.

---

## 1. The `XX-<slug>` Deterministic Prefixing System

To maintain deterministic sorting and avoid ambiguity across operating systems, every file and directory within structured modules (`.ai-memory/plans/`, `01-prompts/`, `03-ai-scripts/`, `spec/`) MUST follow the two-digit zero-padded numeric prefix:
