...
- [`.ai-memory/memory/learned/06-leaf-enums-and-baseenumer-parse-helpers.md`](.ai-memory/memory/learned/06-leaf-enums-and-baseenumer-parse-helpers.md) — Root cause analysis on Go circular import cycles when enum packages import `result` or `errtype`.
- [`.ai-memory/strictly-avoid.md`](.ai-memory/strictly-avoid.md) — Mandatory rule: enums must remain pure leaf packages returning `(Variant, bool)` without higher-level result wrappers.
