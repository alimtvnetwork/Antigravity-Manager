# Plan 25: Email Dispatch, Mailbox Remote Management, Split Security Vault DB, and Bidirectional Remote Control
...
- [x] **Task 1: Split Email Vault Database (`email_vault.db`) & Asymmetric RSA Storage**
  - Implement a dedicated SQLite database module `src-tauri/src/modules/email_vault_db.rs` isolated from general application DBs.
  - Apply WAL mode, 5000ms busy timeout, and relational foreign keys.
  - Store email account aliases, email addresses, SMTP hosts/ports, IMAP hosts/ports, encryption types (`TLS`, `STARTTLS`, `SSL`), active/default flags.
  - Store passwords in a separate credentials table using SSH RSA / asymmetric key encapsulation with salted irreversible derivation.
  - Store notification recipient emails and recipient groups.

- [x] **Task 2: Two-Way Configuration & Mailbox Import/Export (JSON, CSV, Excel XLSX, SQLite DB)**
  - Implement export and import functionality for mailboxes and notification recipients in:
    1. Structured JSON format.
    2. Comma-separated values (CSV) format.
    3. Microsoft Excel (XLSX) format.
    4. SQLite DB backup/restore (`email_vault.db`).
  - Provide safe validation and error handling so corrupted files do not damage existing configurations.

- [x] **Task 3: Outbound Mailer Service with Default Sender & Mailbox Pool Swapping**
  - Create `src-tauri/src/modules/email_sender.rs` implementing SMTP message delivery with clean MIME formatting and anti-spam header encoding.
  - Designate a `default` mailbox sender.
  - If the default sender fails to send (e.g. SMTP auth error or rate limit), automatically swap to the next active mailbox in the pool (failover pool rotation).
  - Support sending to multiple recipient addresses or recipient groups.

- [x] **Task 4: Background Watcher Daemon & Multi-Trigger Telemetry Sensor**
  - Create `src-tauri/src/modules/email_watcher.rs` running a configurable background timer (default: 3 minutes, range: 1–5 min).
  - Capture machine telemetry: hostname and local IP address (with GitMap IP identification).
  - Trigger 1: Monitor active account quota/credit; if quota drops below threshold (e.g., <15%), dispatch an alert email to the notify list; skip if unconfigured.
  - Trigger 2: Notify immediately before an automated workspace/profile switch.
  - Trigger 3: Monitor running projects via `repo_db.rs`; if running projects have no active prompts (idle state), send an alert email:
    *"There is no prompt running. Would you like to send something to these projects? [Project A, Project B]..."*

- [x] **Task 5: Inbound IMAP Mailbox Reader & Bidirectional Remote Execution Bridge**
  - Create `src-tauri/src/modules/email_inbound.rs` running a configurable inbox poller (default: 1 minute).
  - Check the last 5 unread/recent emails from the active IMAP inbox.
  - Parse subject lines and content matching:
    1. `Project: <project-name>`: Injects user's reply prompt directly into the running project workspace in Antigravity.
    2. `exec: <ip>` / `command: <ip>`: Executes approved shell/GitMap instructions on the target machine IP, captures output, and emails back a clean HTML log.
    3. `instance: new` / `instance: create`: Dynamically clones or launches an Antigravity IDE instance.
    4. `rotate: accounts`: Triggers instant account rotation to the next highest-quota profile.
    5. `status` / `query`: Replies with current running projects, active prompts, and account quota table.
    6. `help`: Generates and replies with a complete HTML command cheat sheet with concrete examples.

- [x] **Task 6: Tauri IPC Commands & Error Handling Architecture**
  - Expose Tauri IPC handlers in `src-tauri/src/commands/email.rs` (and `ipc.rs`):
    - `get_email_settings`, `save_email_settings`
    - `list_email_accounts`, `add_email_account`, `update_email_account`, `delete_email_account`, `set_default_email_account`
    - `list_notify_recipients`, `add_notify_recipient`, `delete_notify_recipient`
    - `test_smtp_connection`, `test_imap_connection`
    - `export_email_data` (JSON, CSV, XLSX), `import_email_data`
    - `get_email_watcher_status`, `trigger_manual_email_check`
  - Wrap all errors in standard `AppError` envelopes with stack trace preservation and registered error codes (`E8001`–`E8015`).

- [x] **Task 7: React 19 Frontend Settings UI (`EmailNotificationSettings.tsx`)**
  - Implement a dedicated "Email & Mailbox Automation" panel in `src/pages/Settings.tsx` (or `src/components/settings/EmailSettings.tsx`).
  - Account dialog for adding/editing email credentials (alias, email, password, SMTP/IMAP host, port, encryption type).
  - Default sender selector and mailbox pool status.
  - Notify recipients manager (multiple emails / groups).
  - Configurable polling intervals (telemetry poller, inbox poller) and threshold toggles (quota drop, workspace switch, idle project alert).
  - Clean Import/Export modal with JSON, CSV, and XLSX support.
  - Integrated error modal and test-connection actions with clear feedback.

- [x] **Task 8: Unit Testing & CI Verification**
  - Comprehensive unit tests in `src-tauri/src/modules/email_vault_db.rs`, `email_sender.rs`, `email_inbound.rs`, and `email_watcher.rs`.
  - Validate database schemas, encryption round-trip, CSV/JSON serialization, subject parser matching, and failover rotation.
  - Verify zero lint/formatting errors (`cargo fmt`, ESLint) without running banned local runners.
