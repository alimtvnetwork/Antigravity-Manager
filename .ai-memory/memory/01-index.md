...
- [Rustfmt & Rust Compilation Resolution](issues/28-rustfmt-iterative-codec-and-compile-errors.md) — Root cause analysis and resolution for iterative codec match formatting and email watcher/command queue compilation fixes.
...
- [v4.18.0 Email Management, Split Security Vault DB & Pipeline Resilience](learned/16-v4-18-0-email-management-split-security-db-and-pipeline-resilience.md) — Ingestion of v4.18.0 release state, email subsystem, split security passwords DB, 20 CI/CD RCAs, last 10 git commits, and CODE RED rules.
- [GitMap Fast Search & LLM Commands Mandate](learned/17-gitmap-search-and-llm-commands-protocol.md) — Elimination of broad recursive OS filesystem scans in favor of GitMap indexed search and bounded discovery.
- [Ruby Homebrew Cask Architecture & Ecosystem Role](learned/18-ruby-homebrew-cask-architecture.md) — Architecture and distribution rationale for Homebrew Cask package definitions in macOS/Linuxbrew releases.
- [AGM Dedicated Domain Skills Suite](learned/19-agm-dedicated-domain-skills-suite.md) — Establishment of 6 specialized domain skills for Antigravity-Manager covering reverse proxy, thinking store, multi-instance sandboxing, split SQLite databases, React UI, and email remote control.
- [Issue 32 CI Test Concurrency Race and Windows Entrypoint](issues/32-ci-test-data-dir-race-and-windows-entrypoint.md) — Root cause analysis and fix for multi-threaded test directory collisions, Windows CI entrypoint 0xc0000139, and platform-conditional imports.
