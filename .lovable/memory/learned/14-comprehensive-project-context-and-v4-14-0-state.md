...
> **Path:** `.lovable/memory/learned/14-comprehensive-project-context-and-v4-14-0-state.md`
...
2. `8b04e85` - `docs(plans): document completed plan 21 for v4.14.0 release`:
   - *Files Changed:* `.lovable/plans/01-index.md` (+1), `.lovable/plans/completed/21-ubuntu-ide-diagnostics-and-multi-instance-cloning.md` (+75)
...
8. `b494887` - `docs(plans): consolidate plan 20 subtasks into completed archive`:
   ...
   - *Architectural Intent:* Archives completed subtasks of Plan 20 into `.lovable/plans/completed/`.
...
10. `47c8802` - `docs(cicd): add RCA for TS2503 JSX namespace and TS6133 unused variables`:
    - *Files Changed:* `.lovable/cicd-issues/09-ts-jsx-namespace-and-unused-vars-rca.md` (+42)
...
1. **No Explicit True Checks (TOTAL BAN):** Never evaluate booleans against `true` (`if isReady == true`). Always evaluate implicitly (`if isReady`). Justified in `AGENTS.md` (Rule 1) and `.lovable/strictly-avoid.md`.
...
3. **Never Disable CI/CD (TOTAL BAN):** Never bypass, comment out, or disable CI/CD steps or linters. Justified in `AGENTS.md` (Rule 2) and `.lovable/strictly-avoid.md`.
4. **Strict Lowercase File Naming (TOTAL BAN on uppercase):** All files and scripts must use strictly lowercase naming (e.g. `readme.md`). Justified in `AGENTS.md` (Rule 4) and `.lovable/strictly-avoid.md`.
5. **Strict Relative Git Paths Mandate (TOTAL BAN on Absolute Paths / `file:///` URIs):** Never write absolute paths or `file:///` URIs in any repository files, plans, or documentation. Justified in `AGENTS.md` (Rule 5) and `.lovable/strictly-avoid.md`.
6. **Go Error Handling Standard (`*appfault.AppError`):** Functions returning structured error metadata in Go must use `*appfault.AppError`. Justified in `AGENTS.md` (Rule 6), `.lovable/strictly-avoid.md`, and `.lovable/memory/learned/12-go-cli-apperror-and-dry-help-handling.md`.
7. **`readme.txt` Timestamp Generator (TOTAL BAN):** Never build, suggest, or discuss timestamp generators targeting `readme.txt`. Justified in `.lovable/strictly-avoid.md`.
8. **No Commit of Generated Test Artifacts or Binaries:** `.gitignore` must exclude `.test-report.*`, `.exe`, `.dll`, `tmp/`. Justified in `.lovable/strictly-avoid.md`.
9. **No Release on Every Commit:** Only perform releases when the user explicitly commands it. Justified in `.lovable/strictly-avoid.md`.
10. **Spec/19 Implementation (TOTAL BAN):** Repo is spec-only for `02-spec/19-main-worker-service/`. Justified in `.lovable/strictly-avoid.md`.
...
### Pending Plans (`.lovable/plans/pending/`):
...
### Active Subtask Batches (`.lovable/plans/subtasks/`):
